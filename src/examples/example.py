...
from emoconnect.ble.util import BleRequestUtil, CommandUtil
import logging
...
async def main():
    async def _handle_rx(data: Union[None, Any], is_command: bool):
        print(f'received: {data}')

    emc = EmoConnectManager(_handle_rx)
    await emc.connectToEmo()

    await emc.sendRequest(BleRequestUtil.request([1, 7]))
    await asyncio.sleep(0.5)

    await emc.sendRequest(BleRequestUtil.deviceid())
    await asyncio.sleep(0.5)

    await emc.sendRequest(CommandUtil.statusOnOff(True))
    await emc.sendRequest(CommandUtil.chargeLevelOnOff(True))
